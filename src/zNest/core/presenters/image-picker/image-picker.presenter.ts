...
@Component({selector: 'app-image-picker', templateUrl: './image-picker.presenter.html', styleUrls: ['./image-picker.presenter.scss']})
export class ImagePickerPresenter implements OnInit {
    ...
    constructor(private platform : Platform, private alertSvc : AlertService) {}
        ...
        fr.onload = () => {
            ...
            if (hasAllowedTypes.length == 0) {
                this.alertSvc.showError({
                    msg: "Image types allowed are:" + allowedTypes.toString(),
                    title: "Not Allowed!"
                });
                this.isLoading = false;
            } else {
